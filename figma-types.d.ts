...
type PaintTypeGraident =
  | 'GRADIENT_LINEAR'
  | 'GRADIENT_RADIAL'
  | 'GRADIENT_ANGULAR'
  | 'GRADIENT_DIAMOND';

type PaintTypeImage = 'IMAGE' | 'EMOJI'; // I'm guessing that EMOJI is like an image, not sure where it is used

type TextType = 'TEXT';

type PaintType = PaintTypeSolid | PaintTypeGraident | PaintTypeImage;
...
interface Canvas extends Global {
  ...
  /** An array of settings representing images to from the canvas */
...
interface FrameBase extends Global {
  ...
  readonly backgroundColor: Color;
  /**
   * An array of settings representing images to from node
   * @default []
   */
  readonly exportSettings?: ReadonlyArray<ExportSetting>;
  /**
...
interface BooleanGroup extends VectorBase {
...
interface Slice extends Global {
  readonly type: 'SLICE';
  /** An array of settings representing images to from this node */
  readonly exportSettings: ReadonlyArray<ExportSetting>;
  /** Bounding box of the node in absolute space coordinates */
  readonly absoluteBoundingBox: Rect;
  /**
...
/** Format and size to an asset at */
...
interface TeamComponentsResponse extends PaginationResponse {
  readonly components: ReadonlyArray<FullComponentMetadata>;
}

interface FileComponentsResponse extends PaginationResponse {
  readonly components: ReadonlyArray<FullComponentMetadata>;
}

interface TeamStylesResponse extends PaginationResponse {
  readonly styles: ReadonlyArray<FullStyleMetadata>;
}

interface FileStylesResponse extends PaginationResponse {
  readonly styles: ReadonlyArray<FullStyleMetadata>;
}
